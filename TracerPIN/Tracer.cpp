...
enum LogTypeType { HUMAN, SQLITE, BITS, BYTES };
...
KNOB<string> KnobLogType(KNOB_MODE_WRITEONCE, "pintool",
                         "t", "human", "log type: human/sqlite/bits/bytes");
...
static VOID RecordMem(ADDRINT ip, CHAR r, ADDRINT addr, INT32 size, BOOL isPrefetch)
{
    ...
    switch (LogType) {
        ...
        case SQLITE:
            ...
            break;
        case BITS:
            RecordMemBits(ip, r, addr, memdump, size, isPrefetch);
            break;
        case BYTES:
            RecordMemBytes(ip, r, addr, memdump, size, isPrefetch);
            break;
    }
    PIN_ReleaseLock(&lock);
}
...
int  main(int argc, char *argv[])
{
    ...
    else if (KnobLogType.Value().compare("bits") == 0)
    {
        LogType = BITS;
    }
    else if (KnobLogType.Value().compare("bytes") == 0)
    {
        LogType = BYTES;
    }
